...
class NoPasswordGrant extends AbstractGrant
{

    protected $identifier = 'no_password';
    ...
    public function completeFlow() {
        ...
        $fb_id = $this->server->getRequest()->request->get('facebook_id', null);
        if (is_null($fb_id)) {
            throw new InvalidRequestException('facebook_id');
        }

        $fb_tk = $this->server->getRequest()->request->get('facebook_token', null);
        if (is_null($fb_tk)) {
            throw new InvalidRequestException('facebook_token');
        }

        $userId = call_user_func($this->getVerifyCredentialsCallback(), $email, $fb_id, $fb_tk);
